...
//初始化OV2640 
//参数 MID 保存读取出来的 Manufacturer ID 
//参数 PID 保存读取出来的 Product ID 
//返回值:0,成功
//    其他,错误代码
u8 OV2640_Init(u16 *MID,u16 *PID)
{ 
  GPIO_InitTypeDef  GPIO_InitStructure;
  RCC_AHB1PeriphClockCmd(OV2640_PWDN_LCK|OV2640_RST_LCK, ENABLE);

  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;   //推挽输出
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;  //推挽输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;//100MHz
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;    //上拉
  GPIO_InitStructure.GPIO_Pin = OV2640_PWDN_PIN;  //推挽输出
  GPIO_Init(OV2640_PWDN_GPIO, &GPIO_InitStructure);//初始化

  GPIO_InitStructure.GPIO_Pin = OV2640_RST_PIN;  //推挽输出
  ...
  delay_ms(50); 
  *MID=SCCB_RD_Reg(OV2640_SENSOR_MIDH);	  //读取厂家ID 高八位
  *MID<<=8;
  *MID|=SCCB_RD_Reg(OV2640_SENSOR_MIDL);  //读取厂家ID 低八位
  if(*MID!=OV2640_MID){
    printf("MID:%d\r\n",*MID);
    return 1;
  }
  *PID=SCCB_RD_Reg(OV2640_SENSOR_PIDH);    //读取厂家ID 高八位
  *PID<<=8;
  *PID|=SCCB_RD_Reg(OV2640_SENSOR_PIDL);   //读取厂家ID 低八位
  if(*PID!=OV2640_PID){
    printf("PID:%d\r\n",*PID);
    return 2;
  }
...
//采用SXGA分辨率(1600*1200)  
void OV2640_Resolution_1600_1200(void) 
{
  u16 i=0;
...
//采用SVGA分辨率(SVGA 800*600)
void OV2640_Resolution_800_600(void) 
{
  u16 i=0;
